...
export function ApraGrcBadge({
  ...
  // Critical/High classification should trigger a notification assessment workflow.
  ...
  return (
    <Badge variant="outline" className="text-green-600 border-green-600">
      <Shield className="w-3 h-3 mr-1" />
      Assessed, no material impact
    </Badge>
  );
}

export function CPS234ClassificationBadge({
  classification,
}: {
  ...
  const variants: Record<string, { color: string; label: string }> = {
    ...
    HIGH: {
      ...
      label: "High Impact (72h assessment)",
    },
    CRITICAL: {
      color: "bg-red-100 text-red-800",
      label: "Critical (expedite assessment)",
    },
  };

  const { color, label } = variants[classification] || {
    color: "bg-gray-100",
